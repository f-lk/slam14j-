...
//    waitKey(0);
    ...
    //-- 三角化
    vector<Point3d> points;
    triangulation( keypoints_1, keypoints_2, matches, R, t, points );//根据两张图片的关键点的信息，和R，t，计算得出  关键点的三维信息，放到points中
    
    //-- 验证三角化点与特征点的重投影关系
    Mat K = ( Mat_<double> ( 3,3 ) << 520.9, 0, 325.1, 0, 521.0, 249.7, 0, 0, 1 );
    for ( int i=0; i<matches.size(); i++ )
    {
        Point2d pt1_cam = pixel2cam( keypoints_1[ matches[i].queryIdx ].pt, K );//××像素坐标转化为相机坐标//×××keypoints_1[ matches[i].queryIdx ].pt  是   已经匹配好的第matches[i].queryIdx点的坐标
        Point2d pt1_cam_3d(
            points[i].x/points[i].z, //×××在相机坐标系的归一化平面上
...
Point2f pixel2cam ( const Point2d& p, const Mat& K )//×××
{
    return Point2f//存放的是一个二维坐标float型
    (
        ( p.x - K.at<double>(0,2) ) / K.at<double>(0,0), //×××××将像素坐标系下的横坐标转化为相机坐标系下的横坐标
        ( p.y - K.at<double>(1,2) ) / K.at<double>(1,1) //×××将像素坐标系下的纵坐标转化为相机坐标系下的纵坐标
    );
}
